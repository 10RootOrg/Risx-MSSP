...
# Step 3: Copy template.env to .env if needed
if [ ! -f "misp-docker/.env" ] && [ -f "misp-docker/template.env" ]; then
  cp misp-docker/template.env misp-docker/.env
fi

# Step 4: Load environment variables if .env exists
if [ -f "misp-docker/.env" ]; then
  replace_envs "misp-docker/.env"
fi

# Step 5: Start the service from within misp-docker
